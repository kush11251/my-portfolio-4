...
const GridScan = ({
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (enableGyro && typeof window !== 'undefined' && 'DeviceOrientationEvent' in window && (DeviceOrientationEvent as any).requestPermission) {
        try {
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      renderer.setSize(container.clientWidth, container.clientHeight);
      (uniforms.iResolution.value as THREE.Vector3).set(container.clientWidth, container.clientHeight, renderer.getPixelRatio());
    ...
    const start = async () => {
      ...
      const detect = async (ts: number) => {
        ...
        if (ts - lastDetect >= 33) {
          lastDetect = ts;
          try {
            const res = await (faceapi as any).detectSingleFace(video, opts).withFaceLandmarks(true);
        ...
        if ('requestVideoFrameCallback' in HTMLVideoElement.prototype) {
          (video as any).requestVideoFrameCallback((_frame: number) => detect(performance.now()));
        } else {
  ...
  return (
    <div ref={containerRef} className={`${styles.gridScan}${className ? ` ${className}` : ''}`} style={style}>
      {showPreview && (
        <div className={styles.gridScan__preview}>
          <video ref={videoRef} muted playsInline autoPlay className={styles.gridScan__video} />
          <div className={styles.gridScan__badge}>
...
function median(buf: number[]) {
  ...
  return buf.length % 2 ? a[mid] : (a[mid - 1] + a[mid]) * 0.5;
}
